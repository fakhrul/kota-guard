...
const addResidentToCommunity = (residentId, communityId) => {
  new Promise((resolve, reject) => {
    Community.update(
      { _id: communityId },
...
const updateResidentWithCommunity = (residentId, communityId) => {
  new Promise((resolve, reject) => {
    User.update(
      { _id: residentId },
...
const resolvers = {
  ...
  addResident: (_, args) => {
    Promise.all([
      addResidentToCommunity(args.residentId, args.communityId),
      updateResidentWithCommunity(args.residentId, args.communityId),
    ]).then((result) => result[0]);
